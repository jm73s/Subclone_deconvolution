...
Pyclone-vi is run iteratively, after each run problematic subclones and mutations are identified and remove before the next run. Iteration stops when there is nothings left to remove. Problematic mutations are those that seem related to recombination and problematic subclones are those that are either neglegible or that seem to capture diversity related to an inacurate reference used during allignment.
